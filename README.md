...
- **Anúncios** (AdMob, `src/ads`): banner na colinha, intersticial ao adicionar candidato (no máximo 1 a cada 90 s) e abertura do app (ao abrir e ao voltar; nunca na primeira abertura nem na volta de compartilhar/PDF/links, intervalo de 30 min; regras em `src/ads/policy.ts`). São nativos: não funcionam no Expo Go nem na web. Os IDs reais de Android e iOS estão só nos builds `preview`/`production` do [eas.json](packages/vc-app/eas.json); o build de desenvolvimento usa os IDs de teste do Google (não clique em anúncio real do próprio app). No iPhone, o pedido de rastreamento (ATT) aparece ao sair das boas-vindas, antes de carregar anúncios. `app-ads.txt` em https://nexti.dev/app-ads.txt.
